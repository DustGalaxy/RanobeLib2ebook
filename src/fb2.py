...
class FB2Handler(Handler):
    ...
    def _make_chapter(self, slug: str, priority_branch: str, item: ChapterMeta, delay: float) -> list[ET.Element]:
        time.sleep(delay)
    ...
    async def fill_book(self, slug: str, priority_branch: str, chapters_data: list[ChapterMeta]) -> None:
        ...
        for i, item in enumerate(chapters_data, 1):
            tags: list[ET.Element] = self._make_chapter(slug, priority_branch, item)
        ...
        self.book.titleInfo.sequences = [
            (
                ...
                f"Тома c {chapters_data[0].volume} по {chapters_data[-1].volume}",
            )
        ]
